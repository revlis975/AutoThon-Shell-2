...
MAX_CAP_KEY = "per_incident_minutes_max"

# -------- Helpers --------
def round_half_up(value, places):
    return float(Decimal(value).quantize(Decimal(f"0.{ '0'* (places-1) }1"), rounding=ROUND_HALF_UP))
...
def load_policy(policy_path):
    ...
    return {
        ...
        "env_mult": p.get("multipliers", {}).get("by_environment", {}),
        "fail_mult": p.get("multipliers", {}).get("by_failure_type", {}),
        "module_priority": p.get("module_priority_score", {}),
        "cap": p.get("caps", {}).get(MAX_CAP_KEY, None),
    }
...
if __name__ == "__main__":
    policy_path = "TestData/Policy.yaml"
    failures_path = "TestData/Failures.jsonl"
    output_path = "TestResults/plan.json"

    policy = load_policy(policy_path)
    plan = compute_plan(policy, failures_path)
    write_output(plan, output_path)
    print(f"Wrote {len(plan)} incidents to {output_path}")
